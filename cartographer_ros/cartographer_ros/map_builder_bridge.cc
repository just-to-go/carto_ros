...
void MapBuilderBridge::LoadState(const std::string& state_filename,
  ...
  LOG(WARNING)<<"\n map_builder_->LoadState start";
  map_builder_->LoadState(&stream, load_frozen_state);
  LOG(WARNING)<<"\n map_builder_->LoadState end";
}
...
int MapBuilderBridge::AddTrajectory(
    ...
    const TrajectoryOptions& trajectory_options) {
      // 添加轨迹
  //LocalSlamResultCallback = OnLocalSlamResult（）
  // 添加轨迹,并将 OnLocalSlamResult 作为回调函数传入到 cartographer 中,
  // 不断回调建图结果,作为cartographer 和 cartographer_ros 的连接点.
  ...
  LOG(WARNING)<<"\n sensor_bridges_ init"
  <<"\n 每帧雷达细分数量　trajectory_options.num_subdivisions_per_laser_scan = "<<trajectory_options.num_subdivisions_per_laser_scan
  <<"\n 轨迹跟踪坐标系　trajectory_options.tracking_frame = "<<trajectory_options.tracking_frame
  <<"\n node_options_.lookup_transform_timeout_sec = "<<node_options_.lookup_transform_timeout_sec;

  // 初始化　SensorBridge ,将话题数据，转换成 base_link 坐标系下数据，然后再进行操作
  sensor_bridges_[trajectory_id] = absl::make_unique<SensorBridge>(
      trajectory_options.num_subdivisions_per_laser_scan,
      trajectory_options.tracking_frame,
      node_options_.lookup_transform_timeout_sec, tf_buffer_,
      map_builder_->GetTrajectoryBuilder(trajectory_id));     //将创建的 trajectory_builder 传给 sensor_bridge 用于addOdometry()
