...
// 注册发布器，发布　submap_list, node_list, landmark_poses_list, constraint_list
  ...
  LOG(WARNING)
      <<"\n node_options_.publish_tracked_pose = "<<node_options_.publish_tracked_pose;
  ...
  LOG(WARNING)
    <<"\n 定时器时间："
    <<"\n node_options_.submap_publish_period_sec = "<<node_options_.submap_publish_period_sec
    <<"\n node_options_.pose_publish_period_sec = "<<node_options_.pose_publish_period_sec
    <<"\n node_options_.trajectory_publish_period_sec = "<<node_options_.trajectory_publish_period_sec
    <<"\n kConstraintPublishPeriodSec = "<<kConstraintPublishPeriodSec;
}
...
void Node::AddExtrapolator(const int trajectory_id,
  ...
  extrapolators_.emplace(
    ...
    LOG(WARNING)
      <<"\n Node::AddExtrapolator, 为轨迹　"<<trajectory_id<<" 添加推算器："
      <<"\n kExtrapolationEstimationTimeSec = "<<kExtrapolationEstimationTimeSec
      <<"\n gravity_time_constant = "<<gravity_time_constant;
}
...
void Node::AddSensorSamplers(const int trajectory_id,
  ...
  LOG(WARNING)
    <<"\n Node::AddSensorSamplers, 为轨迹　"<<trajectory_id<<" 添加传感器采样器："
    <<"\n options.rangefinder_sampling_ratio = "<<options.rangefinder_sampling_ratio
 ...
 * 添加轨迹功能：
 *  读取参数，获取订阅的传感器数据话题
 *  根据话题，执行cartographer中的轨迹添加
 *  添加轨迹推算器，odom可以提供机器人运动速度；IMU可以提供机器人的姿态
 *  根据参数设置传感器数据采样频率
 *  注册订阅器，接收传感器数据
 *  **/ 
int Node::AddTrajectory(const TrajectoryOptions& options) {
  ...
  // 为该轨迹添加推算器，发布　PublishLocalTrajectoryData　时使用
  ...
  LOG(WARNING)
    ...
    <<"\n kTopicMismatchCheckDelaySec = "<<kTopicMismatchCheckDelaySec;
  ...
  for (const std::string& topic :
    ...
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::LaserScan>(
         ...
         topic});
         LOG(WARNING)
          <<"\n kLaserScanTopic = "<<kLaserScanTopic
          <<"\n topic = "<<topic;
  }
  for (const std::string& topic : ComputeRepeatedTopicNames(
           kMultiEchoLaserScanTopic, options.num_multi_echo_laser_scans)) {
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::MultiEchoLaserScan>(
         ...
         topic});
         LOG(WARNING)
          <<"\n kMultiEchoLaserScanTopic = "<<kMultiEchoLaserScanTopic
          <<"\n topic = "<<topic;
  }
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kPointCloud2Topic, options.num_point_clouds)) {
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::PointCloud2>(
         ...
         topic});
        LOG(WARNING)
          <<"\n kPointCloud2Topic = "<<kPointCloud2Topic
          <<"\n topic = "<<topic;
  }
  ...
  if (options.use_nav_sat) {
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::NavSatFix>(
             ...
             &node_handle_, this),
         kNavSatFixTopic});
    LOG(WARNING)<<"\n kNavSatFixTopic = "<<kNavSatFixTopic;
  }
// 标签数据处理
  if (options.use_landmarks) {
    subscribers_[trajectory_id].push_back(
...
void Node::HandleOdometryMessage(const int trajectory_id,
  ...
  absl::MutexLock lock(&mutex_);
  if (!sensor_samplers_.at(trajectory_id).odometry_sampler.Pulse()) {    //控制采样
    return;
  }

  // 将nav_msgs::Odometry数据，转换成　carto::sensor::OdometryData
  auto sensor_bridge_ptr = map_builder_bridge_.sensor_bridge(trajectory_id);
  auto odometry_data_ptr = sensor_bridge_ptr->ToOdometryData(msg);
  if (odometry_data_ptr != nullptr) {
    // 外插器，添加里程计数据，计算速度，推算位姿
    extrapolators_.at(trajectory_id).AddOdometryData(*odometry_data_ptr);
  }

  // sensor_bridge_　处理里程计数据
  // sensor_bridge_ptr = map_builder_bridge_.sensor_bridge(trajectory_id)
...
void Node::HandleImuMessage(const int trajectory_id,
  ...
  // 调用　sensor_bridge_　中　HandleImuMessage函数，来处理数据
...
void Node::HandleLaserScanMessage(const int trajectory_id,
  ...
  // 调用sensor_bridge_ 中的　HandleLaserScanMessage函数，来处理数据
  map_builder_bridge_.sensor_bridge(trajectory_id)
