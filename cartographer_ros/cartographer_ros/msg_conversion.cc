...
LaserScanToPointCloudWithIntensities(const LaserMessageType& msg) {
  ...
  for (size_t i = 0; i < msg.ranges.size(); ++i) {
    ...
    if (HasEcho(echoes)) {
      ...
      if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
        ...
        **/ 
      //  将laser_scan 数据,转换成点云数据,即由长度和角度,转换成坐标,基准坐标系为 laser,时间为点与雷达帧的相对时间
  ...
  if (!point_cloud.points.empty()) {
    ...
    // 将雷达帧中的点时刻,从以第一个点为基准,转换成以最后一个点为基准的变换.
    for (auto& point : point_cloud.points) {
      point.time -= duration;
    }
  }
  // 雷达帧时间戳,从第一个点的时间转换成最后一个点的时间,这样方便后面计算
...
// 将 LaserScan 数据 , 转换为点云数据,即由原来的长度和角度,转换成现在的坐标,且将时间戳以起点为基准,转换成以终点为基准
